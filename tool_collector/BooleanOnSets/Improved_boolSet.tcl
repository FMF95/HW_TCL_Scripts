			...
			return
				...
				if { [regexp "Element" $firstEntType] } {
					...
					if  { $list_set_Elems == 0 || $list_set_Elems == "" } {
						tk_messageBox -title "Improved Boolean Operations" -message "0 Elements found" -parent .booleanSetsGUI
						return
					}
					...
					if { $::Aerospace::Imp_Boolean_Set::nodelist1 == "" || $::Aerospace::Imp_Boolean_Set::nodelist2 == "" } {
						tk_messageBox -title "Improved Boolean Operations" -message "please select Nodes for both the lists" -parent .booleanSetsGUI
						return;
					}
					...
					if {([llength $::Aerospace::Imp_Boolean_Set::nodelist1] == 0)} {
								tk_messageBox -title "Improved Boolean Operations" -message "No nodes were selected for List A. Please select nodes." -parent .booleanSetsGUI
								return
					   } elseif {([llength $::Aerospace::Imp_Boolean_Set::nodelist2] == 0)} {
								tk_messageBox -title "Improved Boolean Operations" -message "No nodes were selected for List B. Please select nodes." -parent .booleanSetsGUI
								return
					...
					if  { $list_set_Nodes == 0 || $list_set_Nodes == "" } {
						tk_messageBox -title "Improved Boolean Operations" -message "0 Nodes found" -parent .booleanSetsGUI
						return
					}
				    set newSetname [::Aerospace::Imp_Boolean_Set::GetNewName sets "[join $::Aerospace::Imp_Boolean_Set::setname]"]
					# set newSetname [string map {"HWAT" "BOOLEAN"} $newSetname];
...
proc ::Aerospace::Imp_Boolean_Set::GUI { {x -1} {y -1} } {
		...
		hwtk::dialog .booleanSetsGUI \
			...
			-buttonboxpos se \
			-x $x -y $y \
			-title "Improved Boolean Operations" 
